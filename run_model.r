...
## Save initial verison
setorder(pop, hiv, age, male, risk, cd4, vl, circ, prep, condom, art)
pop[, time := 0] ## This is just for temporary unit testing

pop[, count := as.double(floor(count))] ## Test precision

fwrite(pop, col.names=FALSE, file="pop_initial.out")
...
pop[, time := (nsteps - 1)]
setorder(pop, hiv, age, male, risk, cd4, vl, circ, prep, condom, art)
fwrite(pop, col.names=FALSE, file="pop_final.out")
